...
export default function Analytics({ restaurant }: AnalyticsProps) {
  ...
  useEffect(() => {
    fetchAnalytics();
  }, [restaurant, dateFilter, customDateRange]);
  ...
  const generatePDFContent = (data: any[]) => {
    ...
    if (analytics) {
      ...
      content += `Total Sales: ${formatCurrency(analytics.totalSales)}\n`;
      content += `Average Order Value: ${formatCurrency(analytics.averageOrderValue)}\n\n`;
    }
              ...
              <div className="space-y-3 max-h-64 overflow-y-auto">
                {analytics.salesByDate.slice(-10).map((day) => (
                  <div key={day.date} className="flex items-center justify-between">
                    ...
                    <div className="text-right">
                      <div className="text-sm font-medium text-green-600">{formatCurrency(day.sales)}</div>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {analytics.allOrders.slice(0, 50).map((order) => (
                    <tr key={order.id}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{formatCurrency(order.total || 0)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
